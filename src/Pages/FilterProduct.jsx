...
function FilterProduct() {
  ...
  useEffect(() => {
    getProductByCategoryGamme();
  }, []);
  const getProductByCategoryGamme = async () => {
    try {
    ...
    <div className="flex  justify-center items-center flex-col mx-4">
      <div className="container  flex justify-between items-center my-10 text-[20px] rounded-lg">
        <div className="font-bold">
          {category.id} <span className="text-[#f78e39]">{category.id}</span>
        </div>

        <div
          onClick={() => setBlock(!isblock)}
          className=" flex  relative bg-[#f78e39] w-[145px] justify-between items-center gap-4 p-2 cursor-pointer  transition-all ease-in-out rounded-lg"
        >
      ...
      <div className="container  ShowProduct shadow-lg flex justify-between items-center rounded-lg flex-col py-4 px-4">
        <div className="cards  w-full   flex flex-wrap   justify-between  items-center ">
          {/**DEBUT BOUCLE */}
          {productByCategories.map((productByCategory) => (
            <motion.div
              key={productByCategory.id}
              ...
              className="Card my-2 mx-2 w-[345px] max-[1026px]:w-[300px] max-sm:w-[45%] max-[770px]:w-[270px] max-[785px]:w-[300px] max-[700px]:w-[245px] max-[650px]:w-[210px] max-[785px]:bg-red-900  group p-2 shadow-lg  bg-white rounded-lg transform transition-all cursor-pointer  hover:shadow-2xl hover:-translate-y-2  duration-300"
            >
        ...
        <div className="pagination text-white  flex justify-between w-full flex-wrap mt-4">
